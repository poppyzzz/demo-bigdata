...
select fp.gf_nsrsbh rowkey,map('nsrsbh',COALESCE(fp.gf_nsrsbh,''),'nsrmc',COALESCE(fp.gf_nsrmc,'')) nsrxx,
map() nsrkz,
map('nsrzg_dm','998') nsrrd,
map() ckts,
map() hy,
map() fppz,
map('swjg_key',COALESCE(fp.gf_swjg_dm,''),'region_key',COALESCE(s.region_key,''),'swzj_dm',COALESCE(s.swzj_dm,''),
...
from etl.etl_xxfp fp left join dw.dw_dim_swjg s on fp.gf_swjg_dm = s.swjg_key 
left join dw.dw_hbase_nsr nsr on fp.gf_nsrsbh = nsr.rowkey where nsr.rowkey is null 
