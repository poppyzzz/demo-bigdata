...
DATE=$(date -d"$2" +"%Y%m")
sql="select * from (select rownum as ROWIDSTR,t.* from dzdz.dzdz_fpxx_zzsfp t where kpyf=${DATE}) where \$CONDITIONS"
#sql="select * from (select rownum as ROWIDSTR,t.* from dzdz.dzdz_fpxx_zzsfp_${DATE} t where kpyf=${DATE} ) where \$CONDITIONS"
map=JE=double,SE=Double,JSHJ=DOUBLE,KPJH=DOUBLE,RZDKL_RZYF=DOUBLE,KPYF=STRING
bash ${UtilPath}/SqoopJdbc_Dzdz.sh ${dzdz_username} dzdz DZDZ_FPXX_ZZSFP "$sql" $map
